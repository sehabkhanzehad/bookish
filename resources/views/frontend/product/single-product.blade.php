...
<div class="product px-3 py-3">
    <a href="{{ route('front.product.show', [ $product->id ] ) }}" title="{{ $product->name }}" class="text-decoration-none">
        ...
        <div class="ebook_product_label" style="display: block !important;">
            <span class="onsale ebook_text_lebel" style="display: block !important; text-transform: unset; "> পড়ে দেখুন </span>
        </div>
        @endif
        <div class="image">
            ...
            <p class="m-0 p-0 p-price"><del class="text-muted">₹{{ $product->price }} </del><span class="ps-2 text-danger">@if ($product->offer_price)₹{{ $product->offer_price }}@endif </span></p>
            @else
