          ...
          <div class="section-body">
            ...
            <div class="row">
              ...
              <a class="multi_product_delete btn btn-sm btn-danger mb-1" href="{{ route('admin.deleteAllProduct')}}">
                ...
                <div class="col-md-2">
                    <select name="category" class="form-control select2" id="" form="form">
                ...
                <div class="col-md-2">
                    <select name="subject_id" class="form-control select2" form="form">
                        <option value="">{{__('Select Subject')}}</option>
                        @foreach ($subjects as $category)
                            <option value="{{ $category->id }}" {{ request('subject_id') == $category->id ? 'selected' : '' }}>{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <select name="writer_id" class="form-control select2" form="form">
                        <option value="">{{__('Select Writer')}}</option>
                        @foreach ($writers as $category)
                            <option value="{{ $category->id }}" {{ request('writer_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <select name="publication_id" class="form-control select2" form="form">
                        <option value="">{{__('Select Publication')}}</option>
                        @foreach ($publications as $category)
                            <option value="{{ $category->id }}" {{ request('publication_id') == $category->id ? 'selected' : '' }}>{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <select name="filter" class="form-control select2" form="form">
                        <option value="">{{__('Select Status')}}</option>
                        <option value="pre_order" {{ request('filter') == 'pre_order' ? 'selected' : '' }}>Pre-Order</option>
                        <option value="best_seller" {{ request('filter') == 'best_seller' ? 'selected' : '' }}>Best Seller</option>
                        <option value="active" {{ request('filter') == 'active' ? 'selected' : '' }}>Status Active</option>
                        <option value="inactive" {{ request('filter') == 'inactive' ? 'selected' : '' }}>Status Inactive</option>
                    </select>
                </div>
                <div class="col-md-12 text-right my-2">
                    <button type="submit" class="btn btn-primary ml-3" form="form">Search</button>
                </div>
                        ...
                        <table class="table table-striped" id="">
                            <thead>
                                <tr>
                                    ...
                                    <!--<th width="10%">{{__('Active (Recommanded)')}}</th>-->
                            ...
                            <tbody>
                                @php
                                    $globalIndex = ($products->currentPage() - 1) * $products->perPage();
                                @endphp
                                @foreach ($products as $index => $product)
                                    <tr>
                                        ...
                                        <td>{{ ++$globalIndex }}</td>
                                        <td><a target="_blank" href="{{ route('front.product.single_product',[$product->slug]) }}">{{ $product->name }}</a>
                                            @if($product->pre_order)
                                            <span class="badge py-1 badge-info">Pre Order</span>
                                            @endif
                                        </td>
