        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">{{__('admin.banners')}}</div>
            </div>
          </div>

          <div class="section-body">
            <a href="{{ route('admin.banners.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> {{__('admin.Add New')}}</a>
            <div class="row mt-4">
                <div class="col">
                  <div class="card">
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($banners as $index => $banner)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td>
                                            <img src="{{ asset('uploads/banners/'.$banner->image) }}" alt="" class="banner_image" width="100">
                                        </td>
                                        <td> {{$banner->section}}</td>
                                        <td>
                                            @if($banner->status == 1)
                                            <a href="javascript:;" onclick="changeProductCategoryStatus({{ $banner->id }})">
                                        ...
                                        <td>
                                        <a href="{{ route('admin.banners.edit',$banner->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>
...
<script>
    function deleteData(id){
        $("#deleteForm").attr("action",'{{ url("admin/banners/") }}'+"/"+id)
    }
    function changeProductCategoryStatus(id){
        ...
        $.ajax({
            ...
            url:"{{url('/admin/banners-status/')}}"+"/"+id,
