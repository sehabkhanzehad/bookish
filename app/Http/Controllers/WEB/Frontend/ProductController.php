...
class ProductController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $relatedProducts = Product::with('variantItems', 'subject', 'publication', 'writer', 'category', 'subCategory', 'childCategory', 'brand')
                              ->where('category_id', $product->category_id)
                              ->orWhere('subject_id', $product->subject_id)
                              ->orWhere('publication_id', $product->publication_id)
                              ->orWhere('writer_id', $product->writer_id)
                              ->orWhere('id', '<>', $product->id) // Exclude the current product
                              ->take(5)
                              ->get();
...
public function getProductContent(Request $request)
{
    ...
    if ($product && $product->read_file) {
        $content = file_get_contents(public_path($product->read_file));
