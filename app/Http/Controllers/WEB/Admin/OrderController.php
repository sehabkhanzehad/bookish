  ...
  public function addNewProductEntryadd(Request $request){
    if(!auth()->user()->can('admin.addNewProductEntry')){
        ...
        if($variation->type == 'variable') {
          ...
          $html = view('admin.orders.create_table_data2add', compact('variation','pr_id','color_array','size_array'))->render();  
          ...
          $html = view('admin.orders.create_table_data', compact('variation','pr_id','stock_qty'))->render();  
          return response()->json(['success' => true, 'html' => $html, 'pr_id' => $pr_id]);
        }
    }
    ...
    public function addNewProductEntryedit(Request $request) {
        ...
        if($variation->type == 'variable') {
          ...
          $html = view('admin.orders.create_table_data2edit', compact('variation','pr_id','color_array','size_array'))->render();  
          return response()->json(['success' => true, 'html' => $html, 'pr_id' => $pr_id]);
        } else {
          $html = view('admin.orders.create_table_data_two', compact('variation','pr_id'))->render();  
          return response()->json(['success' => true, 'html' => $html, 'pr_id' => $pr_id]);
        }
    }
