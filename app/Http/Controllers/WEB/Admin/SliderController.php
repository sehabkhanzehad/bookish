...
class SliderController extends Controller
{
    ...
    public function update(Request $request, $id){
        ...
        if($request->slider_image){
            ...
            $slider_image_path = 'uploads/custom-images/'.$slider_image;
            $image = Image::make($request->file('slider_image'));
            $image->resize(1116,223);
            $image->save(public_path() . '/' . $slider_image_path);
            $slider->image = $slider_image_path;
            $slider->save();
        ...
        if($request->slider_image_mobile){
            
            $existing_slider = $slider->mobile_image;
            $extention = $request->slider_image_mobile->getClientOriginalExtension();
            $slider_image_mobile = 'slider'.date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            
            $slider_image_path_mobile = 'uploads/custom-images-mobile/'.$slider_image_mobile;
            $image = Image::make($request->file('slider_image_mobile'));
            $image->resize(400,200);
            $image->save(public_path() . '/' . $slider_image_path_mobile);
            $slider->mobile_image = $slider_image_path_mobile;
            $slider->save();
