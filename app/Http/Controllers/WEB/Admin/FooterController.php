...
class FooterController extends Controller
{
    ...
    public function update(Request $request, $id){
        ...
        if ($request->hasFile('apk')) {
            ...
            $apkFileName = time() . '_' . Str::random(8) . '.' . $apkFile->getClientOriginalExtension();
    
            // Move the uploaded file to a designated directory
            $catelogFilePath = $apkFileName;
            $apkFile->move(public_path('product_files'), $apkFileName);
            $footer->apk = $apkFileName;
    
            // Log successful file upload
            \Log::info('Digital file uploaded successfully: ' . $apkFileName);
            
            $footer->save();
            
            if($old_apk){
                if(File::exists(public_path().'/product_files'.$old_apk))unlink(public_path().'/product_files'.$old_apk);
            }
        }
        
        if ($request->hasFile('catelog')) {
            $old_catelog = $footer->catelog;
            $catelogFile = $request->file('catelog');
            $catelogFileName = time() . '_' . Str::random(8) . '.' . $catelogFile->getClientOriginalExtension();
    
            // Move the uploaded file to a designated directory
            $catelogFilePath = $catelogFileName;
            $catelogFile->move(public_path('product_files'), $catelogFileName);
            $footer->catelog = $catelogFilePath;
    
            // Log successful file upload
            \Log::info('Digital file uploaded successfully: ' . $catelogFileName);
            
            $footer->save();
            
            if($old_catelog){
                if(File::exists(public_path().'/product_files'.$old_catelog))unlink(public_path().'/product_files'.$old_catelog);
            }
        }
    ...
    public function updateFooter(Request $request,$id){
        ...
        if($request->show_ebook == 'on'){
            $status=1;
        } else {
            $status=null;
        }
        $footer->show_ebook=$status;
        $footer->save();
        $notification = trans('admin_validation.Update Successfully');
        $notification=array('messege'=>$notification,'alert-type'=>'success');
        return redirect()->back()->with($notification);
