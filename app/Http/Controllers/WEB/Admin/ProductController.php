...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($writer_id){
            $query->where('writer_id', $writer_id);
        }
        if($category_id){
        ...
        if($filter && $filter == 'pre_order'){
            $query->where('pre_order', 1);
        }
        if($filter && $filter == 'best_seller'){
        ...
        return view('admin.product', compact('products', 'orderProducts', 'setting', 'frontend_view', 'categories', 'writers', 'publications', 'subjects'));
    }
    ...
    public function cat_wise_product(Request $request)
    {
        ...
        $setting = Setting::first();
        $orderProducts = OrderProduct::all();
        $writers = Writer::orderBy('order')->get();
        ...
        return view('admin.cat_product', compact('cat_product', 'writers', 'publications', 'orderProducts', 'setting', 'frontend_view', 'categories', 'cat_id'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.create_product', compact('categories', 'writers', 'publications', 'brands', 'specificationKeys', 'sizes', 'colors', 'subjects', 'packages'));
    }
...
// dd($request->productpdf);
        ...
        if ($request->hasFile('read_file')) {
            ...
            $digitalFilePath = 'product_files/' . $digitalFileName;
            $digitalFile->move(public_path('product_files'), $digitalFileName);
            $product->read_file = $digitalFilePath;
    ...
    public function edit($id)
    {
        ...
        return view('admin.edit_product', compact('var_stock', 'colors', 'sizes', 'writers', 'categories', 'brands', 'publications', 'specificationKeys', 'product', 'subCategories', 'childCategories', 'productSpecifications', 'subjects', 'packages'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('read_file')) {
            ...
            $product->read_file = $digitalFilePath;
            if ($old_read) {
                if (File::exists(public_path() . 'product_files/' . $old_read)) unlink(public_path() . 'product_files/' . $old_read);
            }
            // Log successful file upload
            \Log::info('Read file uploaded successfully: ' . $digitalFileName);
        }
